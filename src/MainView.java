...
public class MainView extends BorderPane {
    ...
    public int axisMax = 1;
    private final int initRangePoints = 50;
...
//        out_field1.maxHeight(50);
        ...
        scroll_pane.setPrefHeight(this.getHeight() - control_pane_height);
...
//        this.setAlignment(Pos.CENTER);
        ...
        this.setStyle("-fx-background-color: LIGHTGRAY; -fx-padding: " + border_Pane + ";");
...
//            drawShape(new Text(transX(l.get(x_pos_in_pattern)), transY(l.get(y_pos_in_pattern)), Integer.toString(l.get(x_pos_in_pattern)) + " " + Integer.toString(l.get(y_pos_in_pattern))));
    ...
    public Integer transX(int x) {        return x + axisMax;    }

    public Integer transY(int y) {        return axisMax * 2 - y - axisMax;    }
...
//        drawShape(new Text(transY(0), transX(axisMax), Integer.toString(transX(0)) + ";" + Integer.toString(transY(axisMax))));
    ...
    public void drawPointColor(int x, int y, Color c) {
        Rectangle point = new Rectangle(transX(x), transY(y), point_size, point_size);
        point.setFill(c);
        canvas_pane.getChildren().add(point);
    }

    public void drawCircle(int x, int y, boolean is_amp) {      // Резерв для случая отрисовки кластерных кругов вместо прямоугольников
        if (fieldsIsFill()) {
...
//        circle.setStroke(Color.BLACK);
    ...
    public void drawRectangle(int start_x, int start_y, int end_x, int end_y) {
        if (fieldsIsFill()) {
